...
async function getOne<Y, T extends Entity<Y>>(options: MicroRequestGetOneOptions<Y, T>): Promise<T> {
  ...
  try {
    ...
    // salvo nella cache
    if (options.cache) {
      await RedisCache.set<Y, T>(options.id, _res.data, _cachePrefix);
    }
    // torno la risposta
    return _res.data;
  } catch (e) {
...
async function getMany<Y, T extends Entity<Y>>(options: MicroRequestGetManyOptions<Y, T>): Promise<T[]> {
  ...
  if (options.cache) {
    ...
    if (_cached && _cached.length) {
      console.log(
        '[@quantos/micro-request][Cache] ' + _url + ' retrieved ' + _cached.length + ' / ' + options.ids.length + ' elements from cache'
      );
...
async function get<T>(options: MicroRequestGetOptions): Promise<T> {
  ...
  try {
    ...
    // salvo nella cache
    if (options.cache) {
      await RedisCache.set(_url, _res.data, _cachePrefix);
    }
    // torno la risposta
    return _res.data;
  } catch (e) {
